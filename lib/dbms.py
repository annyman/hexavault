...
def ask_passwd(name, username, password, tag) -> Passwd:
    tags = []
    words = tag.split()  # Split input by spaces
    for word in words:
        if word.startswith('#'):
    ...
    return Passwd(name, username, password, tags, strength, salt, iv)
...
def add_passwd(db: TinyDB, entry: Passwd, master_password: str): # encrypt and add password to db
    key = derive_key_pbkdf2(master_password, entry.salt)
...
def read_passwd(db: TinyDB, master_password: str): # retrieve and decrypt the password:
    for item in db:
        key = derive_key_pbkdf2(master_password, item['salt'])
...
def search_name_tag(names: List[str], tags: List[str], db: TinyDB) -> TinyDB.table:
    ...
    if len(names) == 0:
        result = db.search(Entry.tag.all(tags))
        for entry in result:
    ...
    if len(tags) == 0:
        ...
        for name in names:
            for tag in tags:
                result = db.search(Entry.name == name and Entry.tag.all(tags))
                for entry in result:
